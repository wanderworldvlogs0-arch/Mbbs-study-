  ...
  const processInline = (str: string, key: string) =>
    str.split(/(\*\*.*?\*\*)/g).map((part, i) =>
      part.startsWith("**") && part.endsWith("**") ? (
        <strong key={`${key}-${i}`} className="font-semibold text-slate-900">{part.slice(2, -2)}</strong>
      ) : (
  ...
  return (
    <AppLayout pageTitle="AI Doubt Solver">
      <div className="h-full flex">
        <div className="w-64 border-r border-slate-200 bg-white flex flex-col shrink-0 hidden md:flex">
            ...
            {chats.map((c) => (
              <button
                ...
                className={`w-full text-left px-3 py-2.5 rounded-lg text-sm mb-1 flex items-center gap-2 truncate ${
                  activeChatId === c.id ? "bg-blue-50 text-blue-700" : "text-slate-600 hover:bg-slate-100"
                }`}
              >
          ...
          <div className="flex-1 overflow-y-auto p-6">
            {messages.length === 0 && (
              <div className="h-full flex flex-col items-center justify-center text-center text-slate-400 gap-3">
                ...
                <p className="text-lg font-medium text-slate-600">Ask me anything about your studies</p>
            ...
            <div className="max-w-3xl mx-auto flex flex-col gap-6">
              {messages.map((m) => (
                <div key={m.id} className={`flex gap-3 ${m.role === "user" ? "flex-row-reverse" : ""}`}>
                  ...
                  <div className={`rounded-2xl px-4 py-3 max-w-[85%] text-[15px] ${
                    m.role === "user" ? "bg-slate-900 text-white" : "bg-white border border-slate-200 text-slate-700"
                  }`}>
              ...
              {sending && (
                <div className="flex gap-3">
                  ...
                  <div className="rounded-2xl px-4 py-3 bg-white border border-slate-200 text-slate-400 text-sm">Thinking…</div>
                </div>
              )}
              <div ref={bottomRef} />
            </div>
          ...
          <div className="border-t border-slate-200 bg-white p-4">
            <div className="max-w-3xl mx-auto flex items-end gap-3">
              <textarea
                ...
                rows={1}
                className="flex-1 resize-none border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/50"
              />
              <button
                onClick={handleSend}
                disabled={sending || !input.trim()}
                className="bg-blue-600 hover:bg-blue-700 disabled:bg-slate-300 text-white p-3 rounded-xl"
              >
