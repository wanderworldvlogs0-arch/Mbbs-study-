  ...
  return (
    <AppLayout pageTitle="Video Learning">
      <div className="p-5 md:p-8 max-w-7xl mx-auto flex flex-col gap-6 md:gap-8">
        <div>
          <h1 className="text-2xl md:text-3xl font-extrabold text-slate-900 dark:text-slate-100 tracking-tight">Video Learning</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1.5 text-sm md:text-base">Browse video lessons across all subjects</p>
        </div>
        ...
        {videos === null ? (
          <div className="p-8 text-center text-slate-400 dark:text-slate-500">Loading videos…</div>
        ) : filtered.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
            {filtered.map((video) => (
              ...
              >
                <div className="w-12 h-12 rounded-xl bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 flex items-center justify-center group-hover:scale-110 transition-transform">
                  <PlayCircle size={24} strokeWidth={1.75} />
                </div>
                <div>
                  <div className="text-[10px] font-bold uppercase tracking-wider text-blue-600 dark:text-blue-400 mb-1.5 flex items-center gap-1.5">
                  ...
                  <div className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400 font-medium mt-2">
                    <Clock size={14} className="text-slate-400 dark:text-slate-500" /> {video.durationMinutes} min
                  </div>
                </div>
              </a>
            ))}
