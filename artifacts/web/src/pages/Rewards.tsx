    ...
    <AppLayout pageTitle="Rewards">
      <div className="p-5 md:p-8 max-w-6xl mx-auto flex flex-col gap-8">
        <div>
          ...
          <p className="text-slate-500 mt-1.5">{earnedCount} of {badges.length} badges earned</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-4">
            ...
            <div>
              <p className="text-slate-500 text-xs font-medium">Subjects Mastered</p>
              <p className="text-xl font-bold text-slate-900">{masteredSubjects}</p>
            </div>
          </div>
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-4">
            ...
            <div>
              <p className="text-slate-500 text-xs font-medium">Leaderboard Rank</p>
              <p className="text-xl font-bold text-slate-900">{rank !== null ? `#${rank}` : "Unranked"}</p>
            </div>
          </div>
        </div>
          ...
          {badges.map((b) => (
            <div
              ...
              className={`bg-white rounded-2xl border p-5 flex items-start gap-4 ${
                b.earned ? "border-slate-200 shadow-sm" : "border-dashed border-slate-200 opacity-60"
              }`}
            >
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${b.earned ? b.bg : "bg-slate-100"}`}>
                {b.earned ? <b.icon className={`w-6 h-6 ${b.color}`} /> : <Lock className="w-5 h-5 text-slate-400" />}
              </div>
              <div>
        ...
        {leaderboard.length > 0 && (
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
            ...
            <div className="flex flex-col gap-1">
              {leaderboard.map((l) => (
                <div key={l.userId} className={`flex items-center p-3 rounded-xl ${l.isUser ? "bg-blue-50 border border-blue-100" : ""}`}>
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-semibold text-slate-900">
                      {l.name} {l.isUser && <span className="text-xs font-normal text-blue-600 ml-1">(You)</span>}
                    </p>
                  </div>
                  <div className="font-bold text-blue-600 text-sm">{l.totalCorrect}</div>
                </div>
              ))}
            </div>
          </div>
