...
const THEMES: Record<string, { bg: string; text: string; ring: string; badgeBg: string }> = {
  preclinical: { bg: "bg-blue-50 dark:bg-blue-900/30", text: "text-blue-600 dark:text-blue-400", ring: "text-blue-500", badgeBg: "bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300" },
  paraclinical: { bg: "bg-teal-50 dark:bg-teal-900/30", text: "text-teal-600 dark:text-teal-400", ring: "text-teal-500", badgeBg: "bg-teal-100 dark:bg-teal-900/50 text-teal-700 dark:text-teal-300" },
  clinical: { bg: "bg-green-50 dark:bg-green-900/30", text: "text-green-600 dark:text-green-400", ring: "text-green-500", badgeBg: "bg-green-100 dark:bg-green-900/50 text-green-700 dark:text-green-300" },
};
...
function SubjectGrid({
  ...
  if (!subjects) {
    return <div className="p-8 text-center text-slate-400 dark:text-slate-500">Loading subjects…</div>;
  }
  ...
  return (
    <div className="p-5 md:p-8 max-w-7xl mx-auto flex flex-col gap-6 md:gap-8">
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-5">
        <div>
          <h1 className="text-2xl md:text-3xl font-extrabold text-slate-900 dark:text-slate-100 tracking-tight">MBBS Subjects</h1>
        ...
        <div className="flex items-center gap-3 w-full lg:w-auto">
          <div className="relative flex-1 lg:w-72">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 text-slate-800 dark:text-slate-200 shadow-sm transition-all placeholder:text-slate-400 dark:placeholder:text-slate-500"
            />
          </div>
          <button className="p-2.5 border border-slate-200 dark:border-slate-700 rounded-xl bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors shadow-sm flex-shrink-0">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
        {filtered.map((subject) => {
          ...
          return (
            <div
              ...
              className="bg-white dark:bg-slate-800 rounded-2xl p-5 border border-slate-200 dark:border-slate-700 hover:border-blue-300 dark:hover:border-blue-700 hover:shadow-xl hover:shadow-blue-500/5 transition-all cursor-pointer group flex flex-col relative overflow-hidden"
            >
              ...
              <div className="mt-auto relative z-10">
                ...
                <h3 className="font-bold text-lg text-slate-800 dark:text-slate-100 mb-3 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors line-clamp-1">{subject.name}</h3>

                <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 text-xs text-slate-500 dark:text-slate-400 font-medium">
                  <span className="flex items-center gap-1.5"><BookOpen size={14} className="text-slate-400 dark:text-slate-500" /> {subject.chapterCount} Ch</span>
                  <span className="flex items-center gap-1.5"><FileQuestion size={14} className="text-slate-400 dark:text-slate-500" /> {subject.mcqCount} MCQs</span>
                  <span className="flex items-center gap-1.5"><PlayCircle size={14} className="text-slate-400 dark:text-slate-500" /> {subject.videoCount} Vids</span>
                </div>
              </div>
            </div>
          );
        ...
        {filtered.length === 0 && (
          <div className="col-span-full py-12 text-center text-slate-500 dark:text-slate-400">No subjects found matching your search.</div>
        )}
      </div>
    </div>
  );
...
function SubjectDetailView({ subjectId, onBack }: { subjectId: string; onBack: () => void }) {
  ...
  if (!subject) {
    return (
      <div className="p-5 md:p-8 max-w-5xl mx-auto">
        <button onClick={onBack} className="flex items-center gap-2 text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 mb-6">
  ...
  return (
    <div className="p-5 md:p-8 max-w-5xl mx-auto flex flex-col gap-6 md:gap-8 pb-20">
      <button onClick={onBack} className="flex items-center gap-2 text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 transition-colors w-fit group">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 lg:p-8 border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden">
        <div className="flex flex-col sm:flex-row sm:items-center gap-5 lg:gap-6 relative z-10">
          ...
          <div>
            ...
            <h1 className="text-2xl md:text-3xl lg:text-4xl font-extrabold text-slate-900 dark:text-slate-100 tracking-tight">{subject.name}</h1>
        ...
        <div className="flex items-center justify-between sm:justify-start gap-4 relative z-10 bg-slate-50 dark:bg-slate-900 p-4 rounded-2xl border border-slate-100 dark:border-slate-700">
          <div>
            <div className="text-xs font-semibold text-slate-500 dark:text-slate-400 mb-0.5 sm:text-right">Overall Progress</div>
            <div className="text-lg md:text-xl font-bold text-slate-900 dark:text-slate-100 sm:text-right">{subject.progressPercent}% Complete</div>
          </div>
          <CircularProgress value={subject.progressPercent} colorClass={theme.ring} />
        </div>
      </div>
        ...
        {TABS.map((tab) => (
          <button
            ...
            className={`px-4 md:px-5 py-3 text-sm font-semibold whitespace-nowrap border-b-2 transition-colors
              ${activeTab === tab ? "border-blue-600 text-blue-600 dark:text-blue-400" : "border-transparent text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 hover:border-slate-300 dark:hover:border-slate-600"}
            `}
          >
            {tab}
          </button>
      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between mb-2 px-1">
          <h2 className="text-lg font-bold text-slate-800 dark:text-slate-100">Syllabus Chapters</h2>
          <div className="text-sm font-medium text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 px-3 py-1 rounded-full">{subject.chapters.length} Chapters</div>
        </div>

        {subject.chapters.length === 0 && (
          <div className="text-center text-slate-400 dark:text-slate-500 py-12 bg-white dark:bg-slate-800 border border-dashed border-slate-200 dark:border-slate-700 rounded-2xl">
        ...
        {subject.chapters.map((ch, idx) => (
          <div
            key={ch.id}
            className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-4 lg:p-5 flex flex-col md:flex-row md:items-center justify-between gap-5 hover:border-blue-300 dark:hover:border-blue-700 hover:shadow-lg hover:shadow-blue-500/5 transition-all group"
          >
            <div className="flex items-start md:items-center gap-4 lg:gap-5 w-full md:w-auto">
              ...
              >
                {ch.progressPercent >= 100 ? (
                  ...
                  <Circle className="text-slate-300 dark:text-slate-600 hover:text-blue-400 dark:hover:text-blue-500 transition-colors" size={30} strokeWidth={2} />
                )}
              </button>
              <div className="flex-1">
                <div className="text-[11px] font-bold text-blue-600 dark:text-blue-400 mb-1.5 uppercase tracking-wider">Chapter {idx + 1}</div>
                <h3 className="text-base md:text-lg font-bold text-slate-800 dark:text-slate-100 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors leading-tight">{ch.title}</h3>
                <div className="flex items-center gap-3 md:gap-4 text-xs md:text-sm text-slate-500 dark:text-slate-400 mt-2 font-medium">
                  <span className="flex items-center gap-1.5"><BookOpen size={15} className="text-slate-400 dark:text-slate-500" /> {ch.subChapterCount} topics</span>
                  <span className="w-1.5 h-1.5 rounded-full bg-slate-300 dark:bg-slate-600"></span>
                  <span className="flex items-center gap-1.5"><Clock size={15} className="text-slate-400 dark:text-slate-500" /> {Math.floor(ch.estimatedMinutes / 60)}h {ch.estimatedMinutes % 60}m</span>
                </div>
              </div>
            </div>
  ...
  return (
    <div className="relative flex items-center justify-center w-14 h-14">
      <svg className="w-full h-full -rotate-90" viewBox="0 0 52 52">
        <circle className="text-slate-100 dark:text-slate-700" strokeWidth="4" stroke="currentColor" fill="transparent" r={radius} cx="26" cy="26" />
...
function FilterBadge({ label, count, active, onClick, color }: { label: string; count: number; active: boolean; onClick: () => void; color: string }) {
  return (
    ...
    >
      {label}
      <span className={`ml-2 px-1.5 py-0.5 rounded-md text-[10px] font-bold ${active ? "bg-white/20 text-white" : "bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400"}`}>{count}</span>
    </button>
  );
}
